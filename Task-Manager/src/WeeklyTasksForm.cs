...
{
    public class WeeklyTasksForm : Form
    {
        TextBox txtName;
        TextBox txtNotes;
        TextBox txtSearch;
        ...
        List<WeeklyTask> allTasks = new List<WeeklyTask>();
        string currentFilter = "Pending";

        public WeeklyTasksForm()
        {
            AppStyle.ApplyForm(this, "Weekly Task Manager", 860, 720);
        ...
        void BuildUI()
        {
            Label title = AppStyle.Label("Weekly Task Manager", 30, 20, 430, 40, AppStyle.TitleFont);
            Controls.Add(title);
            ...
            btnDaily = AppStyle.Button("Daily Tasks", 660, 70, 150, 35, AppStyle.Primary);
            ...
            btnAddTask = AppStyle.Button("Add Task", 660, 170, 150, 35, AppStyle.Success);
            ...
            Controls.Add(AppStyle.Label("Weekly Tasks List", 30, 515, 300, 25, AppStyle.HeaderFont));
        ...
        {
            string msg = "";
            msg += "Weekly Tasks - How To Use" + Environment.NewLine;
            msg += "-------------------------" + Environment.NewLine + Environment.NewLine;
            msg += "1. Write the weekly task name." + Environment.NewLine;
            msg += "2. Choose priority and date." + Environment.NewLine;
            msg += "3. Reminder is optional." + Environment.NewLine;
            msg += "4. Delete Task moves the task to Deleted Tasks." + Environment.NewLine;
            msg += "5. Deleted tasks can be restored from Deleted Tasks page." + Environment.NewLine + Environment.NewLine;
            ...
            using (SQLiteConnection cn = new SQLiteConnection(AppData.ConnString))
            {
                try
                {
                    ...
                    SQLiteCommand cmd = new SQLiteCommand(
                        "INSERT INTO Tasks (Name, Priority, Date, Notes, Status) VALUES (@Name,@Priority,@Date,@Notes,@Status)",
                        cn
                    );
                    ...
                    if (chkReminder.Checked)
                    {
                        ...
                        ReminderService.AddReminder(newID, taskName, "Weekly Task", reminderTime);
                    }
        ...
        private void LoadTasks(string filter)
        {
            ...
            string query = "SELECT * FROM Tasks WHERE Status<>'Deleted'";
        ...
        private void BtnSearch_Click(object sender, EventArgs e)
        {
            ...
            foreach (WeeklyTask task in allTasks)
            {
        ...
        private void BtnComplete_Click(object sender, EventArgs e)
        {
            ...
            WeeklyTask selectedTask = (WeeklyTask)lbTasks.SelectedItem;
            ...
            using (SQLiteConnection cn = new SQLiteConnection(AppData.ConnString))
            {
                try
                {
                    ...
                    SQLiteCommand cmd = new SQLiteCommand("UPDATE Tasks SET Status='Completed' WHERE ID=@ID AND Status='Pending'", cn);
                    cmd.Parameters.AddWithValue("@ID", selectedTask.ID);

                    int rows = cmd.ExecuteNonQuery();

                    ReminderService.RemoveReminder(selectedTask.ID, "Weekly Task");
        ...
        private void BtnDelete_Click(object sender, EventArgs e)
        {
            ...
            using (SQLiteConnection cn = new SQLiteConnection(AppData.ConnString))
            {
                try
                {
                    ...
                    SQLiteCommand cmd = new SQLiteCommand("UPDATE Tasks SET Status='Deleted' WHERE ID=@ID", cn);
                    cmd.Parameters.AddWithValue("@ID", selectedTask.ID);
                    cmd.ExecuteNonQuery();
        ...
        public class WeeklyTask
        {
            ...
            public WeeklyTask(int id, string name, string priority, string date, string status)
            {
                ID = id;
                Name = name;
                Priority = priority;
